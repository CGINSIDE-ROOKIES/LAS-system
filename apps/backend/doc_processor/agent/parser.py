...
class ParserConfig(BaseModel):
    ...
    structured_method: Literal["json_mode"] | None = "json_mode"
...
def _build_worker_payload(
    *,
    ...
    if left_idx is None:
        left_unit = None
        left_text = ""
        left_markers.append("[START_OF_FILE]")
    else:
        left_unit = paragraphs[left_idx].unit_id
        left_text, clipped = _clip_to_budget(
        ...
        if clipped:
            left_markers.append("[LEFT_CONTEXT_CLIPPED]")

    if right_idx is None:
        right_unit = None
        right_text = ""
        right_markers.append("[END_OF_FILE]")
    else:
        right_unit = paragraphs[right_idx].unit_id
        right_text, clipped = _clip_to_budget(
        ...
        if clipped:
            right_markers.append("[RIGHT_CONTEXT_CLIPPED]")
    ...
    return {
        ...
        "target": {
            ...
            "parser_signals": target.parser_signals.model_dump(),
        },
        "right_context": {
            "unit_id": right_unit,
...
def _paragraph_worker(state: ParserGraphState):
    ...
    try:
        ...
        parsed = ParagraphReviewResult.model_validate(raw)
        if cfg.log_llm_io:
            _log(
                state,
...
_builder.add_conditional_edges("prepare_runtime", _splitter, ["paragraph_worker", END])
