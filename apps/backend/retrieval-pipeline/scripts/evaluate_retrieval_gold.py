...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-csv", default="", help="query-level evaluation result CSV output path")
...
def row_hit(row: GoldRow, results: list[dict[str, object]]) -> bool:
    ...
    if expected == "law":
        for r in results:
            ...
            law_ok = doc_type == "law" or (row.gold_law and row.gold_law in (law_name + text))
...
def print_report(backend: str, total: dict[str, int], by_intent: dict[str, dict[str, int]]) -> None:
    count = total["count"]
    hit = total["hit"]
    ...
    for backend in backends:
        ...
        if misses:
            ...
            for row, res in misses[: max(0, args.max_misses)]:
                ...
                print(f"    * [{row.intent}] {row.query} -> top1={top_sid} ({top_type})")
